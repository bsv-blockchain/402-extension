...
(() => {
  ...
  function AESGCM(plainText, initializationVector, key) {
    ...
    const authenticationTag = gctr(s2, preCounterBlock, key);
    return {
      result: cipherText,
      authenticationTag
    };
  ...
  function AESGCMDecrypt(cipherText, initializationVector, authenticationTag, key) {
    ...
    const authInput = buildAuthInput(cipherText);
    const s2 = ghash(authInput, hashSubKey);
    const calculatedTag = gctr(s2, preCounterBlock, key);
...
`;
  ...
  var nextRuleId = 1;
  async function addPaymentHeaderRules(tabId, url, headers) {
  ...
  async function handlePayment(tabId, url, sats, serverKey) {
  ...
  chrome.webRequest.onHeadersReceived.addListener(
    (details) => {
      ...
      if (pendingPayments.has(details.tabId)) {
        console.warn(
          `[402-ext] Tab ${details.tabId} already has a pending payment; retry returned 402. Not retrying again.`
        );
  ...
  var STALE_PAYMENT_TIMEOUT_MS = 5 * 60 * 1e3;
  setInterval(() => {
    const now = Date.now();
