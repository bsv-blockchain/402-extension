...
export interface PendingPayment {
  ...
  serverKey: string
  /** DNR session rule ID, set after payment headers are installed (Chrome only) */
  ruleId: number | null
  /** Timestamp when this payment started processing */
  timestamp: number
  /**
   * The requestId of the request that received the 402.
   * Used in Firefox to distinguish the original (402'd) request from the
   * retry: onBeforeSendHeaders fires for both, but we must only inject
   * payment headers into the retry (a different requestId).
   */
  requestId?: string
}
